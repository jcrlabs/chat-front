    ...
    <div className="relative flex min-h-svh items-center justify-center overflow-hidden px-4" style={{ background: 'var(--bg)' }}>
      ...
      <div className="animate-slide-up relative w-full max-w-sm">
        <div className="mb-8 text-center">
          ...
          <p className="mt-1 text-sm" style={{ color: 'var(--text2)' }}>Join jcrlabs chat</p>
        </div>

        <div className="rounded-2xl p-6" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            {field('Username', (
              <input className={inputCls} style={inputStyle} value={username} onChange={(e) => setUsername(e.target.value)} required
                onFocus={(e) => e.currentTarget.style.borderColor = 'var(--primary)'}
                onBlur={(e) => e.currentTarget.style.borderColor = 'var(--border)'} />
            ))}
            {field('Email', (
              <input type="email" autoComplete="email" className={inputCls} style={inputStyle} value={email} onChange={(e) => setEmail(e.target.value)} required
                onFocus={(e) => e.currentTarget.style.borderColor = 'var(--primary)'}
                onBlur={(e) => e.currentTarget.style.borderColor = 'var(--border)'} />
            ))}
            {field('Password', (
              <input type="password" autoComplete="new-password" className={inputCls} style={inputStyle} value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8}
                onFocus={(e) => e.currentTarget.style.borderColor = 'var(--primary)'}
                onBlur={(e) => e.currentTarget.style.borderColor = 'var(--border)'} />
            ))}
        ...
        <p className="mt-4 text-center text-sm" style={{ color: 'var(--text2)' }}>
          ...
          <Link to="/login" className="font-medium" style={{ color: 'var(--primary-h)' }}>Sign in</Link>
        </p>
      </div>
    </div>
  )
